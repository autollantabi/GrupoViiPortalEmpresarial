...
- **Gestión de canjes XCoin:** XC_GestionCanjes (recurso xcoin.admin o xcoin.viewer). Interfaz para la gestión de canjes de XCoin. Permite filtrar por estado, visualizar el historial de cambios de estado por canje y actualizar el estado actual. Al actualizar un estado, se requiere confirmación y opcionalmente un comentario. Los estados disponibles se obtienen dinámicamente de la API.

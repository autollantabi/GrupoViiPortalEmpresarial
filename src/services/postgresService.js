...
export const postgresService = {
  getComunicados: async () => {
    try {
      const response = await axiosInstanceNew.get('/dwh-postgres/comunicados');
  ...
  obtenerVendedoresParametros: async () => {
    try {
      const response = await axiosInstanceNew.get("/dwh-postgres/vendedores-parametros");
      return {
        success: true,
        data: response.data.data || [],
        message: response.data.message || "Error al conectar con el servidor",
      };
    } catch (error) {
  ...
  eliminarLineaNegocio: async (serial) => {
    try {
  ...
  obtenerLineaNegocioParametros: async () => {
    try {
  ...
  actualizarNombreComercial: async (dlnCodigo, data) => {
    try {

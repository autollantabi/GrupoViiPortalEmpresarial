...
/**
 * Componente IconUI para renderizar iconos de múltiples librerías de react-icons
...
export const IconUI = ({
  ...
  const renderIcon = () => {
    if (!name) {
      console.warn("IconUI: No se proporcionó un nombre de icono");
      return null;
    }
    ...
    if (!iconLibrary) {
      console.warn(`IconUI: Librería de iconos no encontrada para el prefijo "${prefix}"`);
      return null;
    }

    const IconComponent = iconLibrary[name];
    if (!IconComponent) {
      console.warn(`IconUI: Icono "${name}" no encontrado en la librería ${prefix}`);
      return null;
    }
